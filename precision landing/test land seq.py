...
expt.csv.write(f"X,Y,Z,rc1,rc2/n ")
...
def throttle_Disarmed_Or_Not(vehicle,serdev,filename):
    ...
    while not throttle_disarmed:
        landing_sequence.run_repositioning(the_connection,throttle_disarmed,serdev,filename)
...
def land_Complete_Or_Not(vehicle):
    ...
    def handle_status_text(self, name, msg):
        hasread = landing_sequence.offsetData(serdev)
...
landing_sequence.resetQuit(the_connection)
