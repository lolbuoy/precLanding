...
def logging(filename,offsetcorrection,rc1,rc2):
    with open(filename, 'a') as csv:
        ...
        csv.write(f"{x_values},{y_values},{z_values},{rc1_values},{rc2_values} ")
...
# Initialize lists to store data for variables x, y, z, rc1, and rc2
    ...
    plt.plot(y, z, marker='s', linestyle='--', color='g', label='z vs y')
    plt.plot(rc1, z, marker='^', linestyle='-.', color='r', label='z vs rc1')
    plt.plot(rc2, z, marker='d', linestyle=':', color='purple', label='z vs rc2')

    plt.xlabel('X, Y, rc1, rc2')
    plt.ylabel('z-value')
    plt.title('z-value vs Variables')
    plt.grid(True)
    plt.legend()
    plt.savefig(filename)

# Show the plot (optionals
    print(f'The plot has been saved as "{filenamecsv}"')'''