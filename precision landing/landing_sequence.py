...
def read_serial_data(serdev):
    with serial.Serial(serdev, 115200) as ser:
        ...
        # Read a whole line and strip any trailing line ending character:
            time.sleep(0.15)
            line = ser.readlines().decode("utf-8")
...
def offsetData(serdev):
    ...
    # Split the message by spaces to extract values
        ...
        if len(values) < 9:
            ...
            return [0,0],False,0,"nothing"
    # Sleep for a while before generating the next message
    time.sleep(0.20)

def offsetCorrection(offsetdata,vehicle,filename):
    ...
    expt.logging(filename,offsetdata,rc_channel1,rc_channel2)
...
def run_repositioning(vehicle, complete,serdev,filename):
    ...
    offsetCorrection(offset,vehicle,filename)#use pidOut if pid controller has to be tested #use offsetCorrectionPrint for just seeing how the values are behaving
    time.sleep(0.33)